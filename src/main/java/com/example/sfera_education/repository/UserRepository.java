...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    @Query(value = """
        SELECT 
            ...
            RANK() OVER (ORDER BY COALESCE(SUM(h.score), 0) DESC) AS rate
        FROM 
            User u
        LEFT JOIN 
            Group g ON u.groupId = g.id
        LEFT JOIN 
            Category c ON g.category.id = c.id
        LEFT JOIN 
            HomeWork h ON h.student.id = u.id AND h.dueDate BETWEEN :startDate AND :endDate
        WHERE 
            u.role = 'ROLE_STUDENT'
            AND (:keyword IS NULL OR LOWER(CONCAT(u.firstname, ' ', u.lastname)) LIKE LOWER(CONCAT('%', :keyword, '%')))
        ...
        ORDER BY 
            score DESC
    """)
    Page<ResRateStudent> findStudentsWithRate(
    ...
    @Query(value = """
        ...
        ORDER BY 
            score DESC
    """)
    Page<ResRateStudent> findStudentsForTeacher(
            @Param("teacherId") Long teacherId,
            @Param("groupId") Integer groupId,
            @Param("startDate") LocalDate startDate,
            @Param("endDate") LocalDate endDate,
